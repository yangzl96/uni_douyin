...
<template>
	<view style="flex: 1;">
		...
		<list alwaysScrollableVertical="true" :pagingEnabled="true" :show-scrollbar="false" @scroll="listScroll" @scrollend="scroll" :scrollable="true">
			...
			<cell :recycle="false" v-for="(item, index) in playerList" :key="index" :data-index="index" :style="{ height: screenHeight + 'px' }">
				...
				<view class="publish-info-box">
					<view class="">
						<text class="publish-info-vloger-name">@{{ item.user.vlogerName }}</text>
						...
						<view class="publish-info-music-box">
							...
							<text class="muisc-words">{{ item.user.vlogerName }}的原声创作</text>
						</view>
					</view>
				...
				<view class="operation-box">
					<view class="operation-face-box"><image :src="item.user.vlogerFace" class="user-face" @click="goUserInfoSeeSee(item.user.vlogerId)"></image></view>
					<image
						v-if="!item.doIFollowVloger && userId != thisVlogerId"
						src="/static/images/icon-follow.png"
						@click="followMe(item.user.vlogerId)"
						class="follow-me"
					></image>
					<view class="like-box">
						...
						<text class="some-counts">{{ getGraceNumber(item.like_counts) }}</text>
					</view>
